...
class battleMain {
  ...
  draw3main(ctx){
    ...
    if(0 < this.sts4base && this.sts4base < 200){
      ...
      this.ch.drawCutin(ctx,this.sts4dx,this.initcnt,this.sts4type);
    }
  ...
  draw3A(ctx){
    if (this.sts5tm-- <= 0) {
      this.keiryaku.setCutinParam(this.sts5ch.getkwiryaku(this.sts5type), this.ch, this.en);
      //this.sts = 4
      return this.draw3(ctx);
    }
    this.ch.commondraw2(ctx,1);
    this.en.commondraw2(ctx,0);
    this.sts5ch.drawCutin(ctx,this.sts5dx,this.initcnt,this.sts5type);
    // カットイン演出
